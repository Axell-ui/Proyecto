...
export function MultiplayerGame({ room, currentUser, onGameEnd, onBackToLobby }: MultiplayerGameProps) {
  ...
  const [teamScores, setTeamScores] = useState<TeamScore[]>(
    [1, 2, 3, 4, 5].map(id => ({ teamId: id, score: 0, consecutiveMatches: 0 }))
  );
  const [roundMatches, setRoundMatches] = useState(0);

  const config = roundConfig[currentRound - 1];
  const teams = [1, 2, 3, 4, 5].filter(teamId =>
    room.players.filter(p => p.teamId === teamId).length === 2
  );
  const isMyTeamTurn = currentUser.teamId === currentTeam;

  // Initialize cards for round
  useEffect(() => {
  ...
  const handleTimeOut = () => {
    ...
    // Reset consecutive matches for current team
    setTeamScores(prev =>
      prev.map(t =>
        t.teamId === currentTeam ? { ...t, consecutiveMatches: 0 } : t
      )
    );
    ...
    if (newFlipped.length === 2) {
      ...
      if (cards[first].symbol === cards[second].symbol) {
        ...
        setTimeout(() => {
          ...
          setTeamScores(prev =>
            prev.map(t => {
              if (t.teamId === currentTeam) {
                const newConsecutive = t.consecutiveMatches + 1;
                const points = 10 + ((newConsecutive - 1) * 5); // Bonus por rachas
                return {
                  ...t,
          ...
          // Actualizar pares y verificar si la ronda terminó
          setRoundMatches(prevMatches => {
            const newMatches = prevMatches + 1;
            if (newMatches >= config.cards / 2) {
              // Ronda completada
              setTimeout(() => {
                if (currentRound >= 5) {
                  onGameEnd();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-950 via-gray-900 to-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-20">
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-purple-600/20 via-pink-600/20 to-cyan-600/20"
          animate={{
            scale: [1, 1.2, 1],
            rotate: [0, 90, 0],
          }}
      ...
      <div className="relative z-10 container mx-auto px-4 py-6">
        ...
        <motion.div
          ...
          className={`mb-6 p-6 bg-gradient-to-r ${teamColors[currentTeam - 1]} rounded-2xl text-center relative overflow-hidden`}
        >
          <motion.div
          ...
          <div className="relative z-10">
            <h3 className="text-2xl mb-2">
              {isMyTeamTurn ? '¡TU TURNO!' : `Turno del Equipo ${currentTeam}`}
            </h3>
            <div className="flex items-center justify-center gap-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>Racha: x{teamScores.find(t => t.teamId === currentTeam)?.consecutiveMatches || 0}</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-9">
            <div 
              ...
              style={{
                gridTemplateColumns: `repeat(${Math.min(6, Math.ceil(Math.sqrt(config.cards)))}, minmax(0, 1fr))`,
              }}
            >
              <AnimatePresence>
                {cards.map((card, index) => (
                  <MemoryCard
                    ...
                    card={card}
                    index={index}
                    glowColor={isMyTeamTurn ? '#00ff88' : '#666'}
          ...
          <div className="col-span-3 space-y-3">
            <div className="bg-gray-800/50 backdrop-blur-xl rounded-xl p-4 border border-gray-700">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-lg">Puntuación</h3>
              </div>
                    ...
                    <motion.div
                      ...
                      className={`p-3 rounded-lg ${
                        team.teamId === currentTeam
                          ? `bg-gradient-to-r ${teamColors[team.teamId - 1]}`
                          : 'bg-gray-700/50'
                      }`}
                    >
            ...
            <div className="bg-gray-800/50 backdrop-blur-xl rounded-xl p-4 border border-gray-700">
              ...
              <div className="w-full h-3 bg-gray-700 rounded-full overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${(roundMatches / (config.cards / 2)) * 100}%` }}
                  transition={{ duration: 0.5 }}
                />
              </div>
              <p className="text-xs text-gray-400 mt-2 text-center">
                {roundMatches} / {config.cards / 2} pares
              </p>
            </div>
          </div>
        </div>
