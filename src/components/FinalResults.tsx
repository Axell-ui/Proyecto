...
export function FinalResults({ room, onBackToLobby }: FinalResultsProps) {
  // Simular puntuaciones finales
  const teams = [1, 2, 3, 4, 5]
    .filter(teamId => room.players.filter(p => p.teamId === teamId).length === 2)
    .map(teamId => ({
      teamId,
      players: room.players.filter(p => p.teamId === teamId),
      score: Math.floor(Math.random() * 300) + 200, // Simular scores
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-purple-950 to-black relative overflow-hidden flex items-center justify-center">
      ...
      <div className="absolute inset-0">
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-yellow-600/20 via-purple-600/20 to-pink-600/20"
          animate={{
            scale: [1, 1.3, 1],
            rotate: [0, 180, 360],
          }}
        ...
        {[...Array(50)].map((_, i) => (
          <motion.div
            ...
            style={{
              ...
              top: `-5%`,
              backgroundColor: ['#ffd700', '#ff4500', '#00d4ff', '#00ff88', '#b19cd9'][
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        >
          ...
          >
            <h1 className="text-7xl mb-4" style={{ textShadow: `0 0 40px ${teamColors[winner.teamId - 1].glow}` }}>
              ¡PARTIDA FINALIZADA!
            </h1>
          ...
          <motion.div
            ...
            className={`mb-12 p-8 bg-gradient-to-r ${teamColors[winner.teamId - 1].bg} rounded-3xl relative overflow-hidden`}
          >
            <motion.div
            ...
            <div className="relative z-10 text-center">
              ...
              <div className="flex items-center justify-center gap-8 text-xl">
                <div className="flex items-center gap-2">
                  ...
                  <span>{winner.matches} Aciertos</span>
                </div>
                <div className="flex items-center gap-2">
                  <Zap className="w-6 h-6" />
                  <span>Mejor Racha: x{winner.bestStreak}</span>
                </div>
              </div>
              <div className="mt-6 space-y-2">
                {winner.players.map((player, idx) => (
                  <div key={idx} className="text-lg opacity-90">
                    👤 {player.email}
                  </div>
                ))}
              </div>
              ...
              return (
                <motion.div
                  ...
                  className={`bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl rounded-2xl p-6 border-2 ${
                    isWinner ? 'border-yellow-500' : isLoser ? 'border-red-500' : 'border-gray-700'
                  }`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-full bg-gradient-to-r ${color.bg} flex items-center justify-center text-2xl`}>
                        {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `#${index + 1}`}
                      </div>
                      <div>
                        <h3 className="text-2xl">Equipo {team.teamId}</h3>
                        <div className="flex items-center gap-2 text-gray-400">
                          {isWinner && (
                            <>
                              <Crown className="w-4 h-4 text-yellow-400" />
                              <span className="text-sm text-yellow-400">Ganador</span>
                            </>
                          )}
                          {isLoser && (
                            <>
                              <TrendingDown className="w-4 h-4 text-red-400" />
                              <span className="text-sm text-red-400">Último lugar</span>
                            </>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="p-3 bg-gray-700/30 rounded-lg text-center">
                      <div className="text-xl">{team.matches}</div>
                      <div className="text-xs text-gray-400">Aciertos</div>
                    </div>
                    <div className="p-3 bg-gray-700/30 rounded-lg text-center">
                      <div className="text-xl">x{team.bestStreak}</div>
                  ...
                  <div className="space-y-2">
                    {team.players.map((player, idx) => (
                      <div key={idx} className="flex items-center gap-2 p-2 bg-gray-700/20 rounded-lg">
          ...
          <motion.div
            ...
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 1 }}
            className="mb-8 p-6 bg-red-900/20 border-2 border-red-500 rounded-2xl text-center"
          >
            <h3 className="text-2xl mb-2 text-red-400">
              Equipo {loser.teamId} - Último Lugar
            </h3>
            <p className="text-gray-400">
              ¡No se desanimen! La práctica hace al maestro. ¡Inténtenlo de nuevo!
            </p>
          </motion.div>
          ...
          >
            <motion.button
              ...
              className="px-12 py-4 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-700 hover:to-purple-700 rounded-2xl text-xl flex items-center gap-3 mx-auto shadow-lg"
            >
